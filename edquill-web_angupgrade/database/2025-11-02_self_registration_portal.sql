...
CALL add_column_if_not_exists(
  'student_self_registration_courses',
  'decision_status',
  "`decision_status` ENUM('pending','approved','waitlisted','declined') NOT NULL DEFAULT 'pending' AFTER `fee_amount`"
);

CALL add_column_if_not_exists(
  'student_self_registration_courses',
  'approved_schedule_id',
  '`approved_schedule_id` BIGINT UNSIGNED NULL AFTER `decision_status`'
);

CALL add_column_if_not_exists(
  'student_self_registration_courses',
  'approved_schedule_title',
  '`approved_schedule_title` VARCHAR(150) NULL AFTER `approved_schedule_id`'
);

CALL add_column_if_not_exists(
  'student_self_registration_courses',
  'approved_schedule_start',
  '`approved_schedule_start` DATE NULL AFTER `approved_schedule_title`'
);

CALL add_column_if_not_exists(
  'student_self_registration_courses',
  'approved_schedule_end',
  '`approved_schedule_end` DATE NULL AFTER `approved_schedule_start`'
);

CALL add_column_if_not_exists(
  'student_self_registration_courses',
  'approved_fee_amount',
  '`approved_fee_amount` DECIMAL(12,2) NULL AFTER `approved_schedule_end`'
);

CALL add_column_if_not_exists(
  'student_self_registration_courses',
  'decision_notes',
  '`decision_notes` VARCHAR(255) NULL AFTER `approved_fee_amount`'
);
...
CREATE TABLE IF NOT EXISTS school_communication_providers (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  school_id BIGINT UNSIGNED NOT NULL,
  channel ENUM('email','sms') NOT NULL,
  ...
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY ux_school_channel_provider (school_id, channel, provider),
  KEY idx_school_communication_providers_school (school_id),
  CONSTRAINT fk_school_communication_providers_school
    FOREIGN KEY (school_id) REFERENCES school(school_id)
      ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
