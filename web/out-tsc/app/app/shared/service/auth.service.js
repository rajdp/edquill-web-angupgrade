...
import * as i5 from "./creator.service";
export class AuthService {
    setAssessmentRef(ref) {
        this.assessmentRef = ref;
    }
    setAssignmentRef(ref) {
        this.assignmentRef = ref;
    }
    setDashboardRef(ref) {
        this.dashboardRef = ref;
    }
    setClassDetailsRef(ref) {
        this.classDetailRef = ref;
    }
    constructor(router, toastr, http, env, creatorService) {
        this.router = router;
        this.toastr = toastr;
        this.http = http;
        this.env = env;
        ...
        this.assessmentRef = null;
        this.assignmentRef = null;
        this.dashboardRef = null;
        this.classDetailRef = null;
        this._sidebarOpened = new BehaviorSubject(true); // default = open
        this.sidebarOpened$ = this._sidebarOpened.asObservable();
    }
    ...
    checkPermission(groupId, id) {
        ...
        const hasPermission = schoolDetails.permissions.some(group => group.group_id == groupId && group.permission.some(p => p.id == id && p.status == 1));
    ...
    getCurrentDateAndTime(contentDetails, calledFrom = '', pageCalledFrom = '') {
        const payload = {
            platform: 'web',
        ...
        return new Observable(observer => {
            this.postService(payload, 'student/getCurrentDateTime').subscribe((successData) => {
    ...
    testDetailsRoutingPage(arrayValue, pageCalledFrom) {
        ...
        arrayValue[0]?.contents.every((value, index) => {
            if (value.content_started_at == null && (value.student_content_status == '1' || value.student_content_status == '2')) {
                ...
                this.getCurrentDateAndTime(value, 'test', pageCalledFrom).subscribe((isAllowed) => {
                    if (!isAllowed) {
    ...
    updateContentStatus(data, calledFrom) {
        ...
        this.postService(payload, 'student/updateContentStartTime').subscribe((successData) => {
    ...
    checkContentAccess(contentStartedAt, contentDuration, currentTime) {
    ...
    isSingleQnsWithFeedbackType(array) {
    ...
    static { this.ɵfac = function AuthService_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || AuthService)(i0.ɵɵinject(i1.Router), i0.ɵɵinject(i2.ToastrService), i0.ɵɵinject(i3.HttpClient), i0.ɵɵinject(i4.EnvironmentService), i0.ɵɵinject(i5.CreatorService)); }; }
...
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(AuthService, [{
    ...
    }], () => [{ type: i1.Router }, { type: i2.ToastrService }, { type: i3.HttpClient }, { type: i4.EnvironmentService }, { type: i5.CreatorService }], null); })();
