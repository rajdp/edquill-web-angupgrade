...
export class TerminologyService {
  ...
  get(path: string): string {
    const value = path.split('.').reduce((obj, key) => obj?.[key], this.terminology as any);
    return value || path;
  }
   ...
   */
  getValidationMessage(type: string, params?: { field?: string; min?: number; max?: number }): string {
   ...
   * Get status label
   */
  getStatusLabel(statusValue: string | number): string {
   ...
   */
  getContactLabel(contactNumber: 1 | 2, format: 'label' | 'short' = 'label'): string {
   ...
   */
  getFormLabels(section: 'student' | 'contact1' | 'contact2'): { [key: string]: string } {
